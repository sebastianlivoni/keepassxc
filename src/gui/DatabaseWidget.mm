...
DatabaseWidget::DatabaseWidget(QSharedPointer<Database> db, QWidget *parent)
    : QStackedWidget(parent), m_db(std::move(db)),
      ...
      m_searchingLabel(new QLabel(this)), m_shareLabel(new ElidedLabel(this)),
      ...
      m_tagView(new TagView(this)), m_saveAttempts(0),
  ...
  m_groupSplitter->setVisible(
      !config()->get(Config::GUI_HideGroupPanel).toBool());
...
DatabaseWidget::~DatabaseWidget() {
  ...
  // explicitly clearing the Database pointer, instead of leaving it to
  // ~QSharedPointer. QSharedPointer may behave differently depending on whether
  // it is cleared by the `clear` method or by its destructor. In the latter
  // case, the ref counter may not be correctly maintained if a copy of the
  // QSharedPointer is created in any slots activated by the Database
  // destructor. More details:
  // https://github.com/keepassxreboot/keepassxc/issues/6393.
  m_db.clear();
}

QSharedPointer<Database> DatabaseWidget::database() const { return m_db; }
...
DatabaseWidget::Mode DatabaseWidget::currentMode() const {
  ...
  if (widget == m_mainWidget) {
    ...
    mode = Mode::DatabaseSettingsMode;
             ...
             widget == m_historyEditEntryWidget) {
...
bool DatabaseWidget::isSaving() const { return m_db->isSaving(); }

bool DatabaseWidget::isSorted() const { return m_entryView->isSorted(); }
...
QString DatabaseWidget::displayName() const {
  ...
  if (!m_db->filePath().isEmpty()) {
    ...
    if (displayName.isEmpty()) {
      ...
      displayName = tr("%1 [New Database]", "Database tab name modifier")
                        .arg(displayName);
    }
  }
...
void DatabaseWidget::setSplitterSizes(
    const QHash<Config::ConfigKey, QList<int>> &sizes) {
  // Set the splitter sizes, if the size is invalid set a default ratio based on
  // this widget size
  for (auto itr = sizes.constBegin(); itr != sizes.constEnd(); ++itr) {
    ...
    case Config::GUI_SplitterState:
      if (value.size() < 2) {
        value = QList({static_cast<int>(width() * 0.25),
    ...
    case Config::GUI_PreviewSplitterState:
      if (value.size() < 2) {
        value = QList({static_cast<int>(height() * 0.8),
    ...
    case Config::GUI_GroupSplitterState:
      if (value.size() < 2) {
        value = QList({static_cast<int>(height() * 0.6),
...
void DatabaseWidget::replaceDatabase(QSharedPointer<Database> db) {
  ...
  // TODO: instead of increasing the ref count temporarily, there should be a
  // clean break from the old database. Without this crashes occur due to the
  // change signals triggering dangling pointers.
  auto oldDb = m_db;
  m_db = std::move(db);
  connectDatabaseSignals();
  ...
  // Restore the new parent group pointer, if not found default to the root
  // group this prevents data loss when merging a database while creating a new
  // entry
...
void DatabaseWidget::showTotp() {
  ...
  connect(this, &DatabaseWidget::databaseLockRequested, totpDialog,
          &TotpDialog::close);
  totpDialog->open();
}
...
void DatabaseWidget::setupTotp() {
  ...
  connect(setupTotpDialog, SIGNAL(totpUpdated()),
          SIGNAL(entrySelectionChanged()));
  if (currentWidget() == m_editEntryWidget) {
    // Entry is being edited, tell it when we are finished updating TOTP
    connect(setupTotpDialog, SIGNAL(totpUpdated()), m_editEntryWidget,
            SLOT(updateTotp()));
  }
  connect(this, &DatabaseWidget::databaseLockRequested, setupTotpDialog,
...
void DatabaseWidget::expireSelectedEntries() {
  const QModelIndexList selected =
      m_entryView->selectionModel()->selectedRows();
...
void DatabaseWidget::deleteEntries(QList<Entry *> selectedEntries,
  ...
  bool permanent = (recycleBin && recycleBin->findEntryByUuid(
                                      selectedEntries.first()->uuid())) ||
                   !m_db->metadata()->recycleBinEnabled();

  if (confirm &&
      !GuiTools::confirmDeleteEntries(this, selectedEntries, permanent)) {
    return;
  }
...
void DatabaseWidget::copyTitle() {
  ...
  if (currentEntry) {
    setClipboardTextAndMinimize(
        currentEntry->resolveMultiplePlaceholders(currentEntry->title()));
  }
}
...
void DatabaseWidget::copyUsername() {
  ...
  if (currentEntry) {
    setClipboardTextAndMinimize(
        currentEntry->resolveMultiplePlaceholders(currentEntry->username()));
  }
}
...
void DatabaseWidget::copyPassword() {
  ...
  if (currentEntry) {
    setClipboardTextAndMinimize(
        currentEntry->resolveMultiplePlaceholders(currentEntry->password()));
  }
}
...
void DatabaseWidget::copyURL() {
  ...
  if (currentEntry) {
    setClipboardTextAndMinimize(
        currentEntry->resolveMultiplePlaceholders(currentEntry->url()));
  }
}
...
void DatabaseWidget::copyNotes() {
  ...
  if (currentEntry) {
    setClipboardTextAndMinimize(
        currentEntry->resolveMultiplePlaceholders(currentEntry->notes()));
  }
}
...
void DatabaseWidget::copyAttribute(QAction *action) {
  auto currentEntry = currentSelectedEntry();
  if (currentEntry) {
    setClipboardTextAndMinimize(currentEntry->resolveMultiplePlaceholders(
        currentEntry->attributes()->value(action->data().toString())));
  }
}

bool DatabaseWidget::copyFocusedTextSelection() {
  // If a focused child widget has text selected, copy that text to the
  // clipboard and return true. Otherwise, return false.

  const bool clearClipboard =
  ...
  if (plainTextEdit && plainTextEdit->textCursor().hasSelection()) {
    clipboard()->setText(plainTextEdit->textCursor().selectedText(),
                         clearClipboard);
    return true;
  }
  ...
  if (textEdit && textEdit->textCursor().hasSelection()) {
    clipboard()->setText(textEdit->textCursor().selection().toPlainText(),
                         clearClipboard);
    return true;
  }
...
void DatabaseWidget::showTotpKeyQrCode() {
  ...
  if (currentEntry) {
    ...
    connect(this, &DatabaseWidget::databaseLockRequested, totpDisplayDialog,
            &TotpExportSettingsDialog::close);
...
void DatabaseWidget::performAutoType(const QString &sequence) {
  ...
  if (currentEntry) {
    ...
    if (config()->get(Config::Security_AutoTypeAsk).toBool() &&
        MessageBox::question(
            this, tr("Confirm Auto-Type"),
            tr("Perform Auto-Type into the previously active window?")) !=
            MessageBox::Yes) {
      return;
    }
...
void DatabaseWidget::performIconDownloads(const QList<Entry *> &entries,
                                          bool force,
                                          bool downloadInBackground) {
#ifdef WITH_XC_NETWORKING
  auto *iconDownloaderDialog = new IconDownloaderDialog(this);
  connect(this, SIGNAL(databaseLockRequested()), iconDownloaderDialog,
          SLOT(close()));
...
void DatabaseWidget::openUrlForEntry(Entry *entry) {
  ...
  if (cmdString.startsWith("cmd://")) {
    ...
    bool launch = (entry->attributes()->value(
                       EntryAttributes::RememberCmdExecAttr) == "1");

    // otherwise ask user
    if (!launch && cmdString.length() > 6) {
      QString cmdTruncated = entry->resolveMultiplePlaceholders(
          entry->maskPasswordPlaceholders(entry->url()));
      cmdTruncated = cmdTruncated.mid(6);
      if (cmdTruncated.length() > 400) {
        cmdTruncated = cmdTruncated.left(400) + " […]";
      }
      QMessageBox msgbox(QMessageBox::Icon::Question, tr("Execute command?"),
                         tr("Do you really want to execute the following "
                            "command?<br><br>%1<br>")
    ...
    QUrl url =
        QUrl::fromUserInput(entry->resolveMultiplePlaceholders(entry->url()));
    if (!url.isEmpty()) {
...
void DatabaseWidget::cloneGroup() {
  ...
  m_newGroup.reset(currentGroup->clone(
      Entry::CloneCopy, Group::CloneDefault | Group::CloneRenameTitle));
...
void DatabaseWidget::deleteGroup() {
  ...
  auto *recycleBin = m_db->metadata()->recycleBin();
  bool inRecycleBin =
      recycleBin && recycleBin->findGroupByUuid(currentGroup->uuid());
  bool isRecycleBin = recycleBin && (currentGroup == recycleBin);
  bool isRecycleBinSubgroup =
      recycleBin && currentGroup->findGroupByUuid(recycleBin->uuid());
  if (inRecycleBin || isRecycleBin || isRecycleBinSubgroup ||
    ...
    auto result = MessageBox::question(
        ...
        MessageBox::Delete | MessageBox::Cancel, MessageBox::Cancel);
    ...
    auto result = MessageBox::question(
        this, tr("Move group to recycle bin?"),
        tr("Do you really want to move the group "
           "\"%1\" to the recycle bin?")
            .arg(currentGroup->name().toHtmlEscaped()),
        MessageBox::Move | MessageBox::Cancel, MessageBox::Cancel);
...
void DatabaseWidget::syncWithRemote(const RemoteParams *params) {
  ...
  result.errorMessage =
      tr("Remote Sync did not contain any download or upload commands.");
...
void DatabaseWidget::syncDatabaseWithLockedDatabase(
    const QString &filePath, const RemoteParams *params) {
  ...
  connect(this, &DatabaseWidget::databaseSyncUnlocked,
          [this, params](const RemoteHandler::RemoteResult &result) {
            uploadAndFinishSync(params, result);
          });
  connect(this, &DatabaseWidget::databaseSyncUnlockFailed,
          [this, params](const RemoteHandler::RemoteResult &result) {
...
void DatabaseWidget::uploadAndFinishSync(const RemoteParams *params,
                                         RemoteHandler::RemoteResult result) {
  QScopedPointer<RemoteHandler> remoteHandler(new RemoteHandler(this));
...
void DatabaseWidget::finishSync(const RemoteParams *params,
                                RemoteHandler::RemoteResult result) {
  setDisabled(false);
  ...
  if (result.success) {
    ...
    showMessage(
        tr("Remote sync '%1' completed successfully!").arg(params->name),
        MessageWidget::Positive, false);
  } else {
    emit databaseSyncFailed(params->name, result.errorMessage);
    showErrorMessage(tr("Remote sync '%1' failed: %2")
                         .arg(params->name, result.errorMessage));
  }
}
...
void DatabaseWidget::switchToHistoryView(Entry *entry) {
  auto entryTitle = m_editEntryWidget->currentEntry()
                        ? m_editEntryWidget->currentEntry()->title()
                        : "";
...
void DatabaseWidget::connectDatabaseSignals() {
  ...
  connect(m_db.data(), &Database::modified, this,
          &DatabaseWidget::databaseModified);
  connect(m_db.data(), &Database::modified, this,
          &DatabaseWidget::onDatabaseModified);
  connect(m_db.data(), &Database::databaseSaved, this,
          &DatabaseWidget::databaseSaved);
  connect(m_db.data(), &Database::databaseFileChanged, this,
          &DatabaseWidget::reloadDatabaseFile);
  connect(m_db.data(), &Database::databaseNonDataChanged, this,
          &DatabaseWidget::databaseNonDataChanged);
  connect(m_db.data(), &Database::databaseNonDataChanged, this,
          &DatabaseWidget::onDatabaseNonDataChanged);
}
...
void DatabaseWidget::loadDatabase(bool accepted) {
  ...
  if (accepted) {
    ...
    if (m_groupBeforeLock.isNull() &&
        config()
            ->get(Config::GUI_ShowExpiredEntriesOnDatabaseUnlock)
            .toBool()) {
      ...
      if (expirationOffset <= 0) {
        ...
        m_nextSearchLabelText =
            tr("Entries expiring within %1 day(s)", "", expirationOffset)
                .arg(expirationOffset);
      }
...
void DatabaseWidget::mergeDatabase(bool accepted) {
  if (accepted) {
    ...
    if (!srcDb) {
      showMessage(tr("No source database, nothing to do."),
                  MessageWidget::Error);
      return;
    }
    ...
    if (!changeList.isEmpty()) {
      showMessage(tr("Successfully merged the database files."),
                  MessageWidget::Information);
    } else {
      showMessage(tr("Database was not modified by merge operation."),
                  MessageWidget::Information);
    }
  }
...
bool DatabaseWidget::syncWithDatabase(const QSharedPointer<Database> &otherDb,
                                      QString &error) {
  ...
  if (!changeList.isEmpty()) {
    ...
    if (!m_db->save(Database::Atomic, {}, &error)) {
      error =
          tr("Error while saving database %1: %2").arg(m_db->filePath(), error);
      return false;
    }
    if (!otherDb->save(Database::Atomic, {}, &error)) {
      error = tr("Error while saving database %1: %2")
                  .arg(otherDb->filePath(), error);
      return false;
    }
  }
  return true;
...
void DatabaseWidget::unlockDatabase(bool accepted) {
  ...
  if (!accepted) {
    ...
    if (senderDialog &&
        senderDialog->intent() == DatabaseOpenDialog::Intent::RemoteSync) {
      RemoteHandler::RemoteResult result;
      result.success = false;
  ...
  if (senderDialog) {
    if (senderDialog->intent() == DatabaseOpenDialog::Intent::Merge) {
      ...
      return;
    } else if (senderDialog->intent() ==
               DatabaseOpenDialog::Intent::RemoteSync) {
  ...
  if (senderDialog &&
      senderDialog->intent() == DatabaseOpenDialog::Intent::AutoType) {
    // Rather than starting AutoType directly for this database, signal the
    // parent DatabaseTabWidget to restart AutoType now that this database is
    // unlocked, so that other open+unlocked databases can be included in the
    // search.
    emit requestGlobalAutoType(m_searchStringForAutoType);
  }
}

void DatabaseWidget::entryActivationSignalReceived(
    Entry *entry, EntryModel::ModelColumn column) {
  Q_ASSERT(entry);
  if (!entry) {
    return;
  }
...
void DatabaseWidget::switchToOpenDatabase() {
  if (currentWidget() != m_databaseOpenWidget ||
      m_databaseOpenWidget->filename() != m_db->filePath()) {
...
void DatabaseWidget::sortGroupsAsc() { m_groupView->sortGroups(); }

void DatabaseWidget::sortGroupsDesc() { m_groupView->sortGroups(true); }
...
void DatabaseWidget::removePasskeyFromEntry() {
  ...
  auto result = MessageBox::question(
      this, tr("Remove passkey from entry"),
...
void DatabaseWidget::saveSearch(const QString &searchtext) {
  ...
  connect(this, &DatabaseWidget::databaseLockRequested, &dialog,
          &QInputDialog::reject);
  ...
  dialog.setLabelText(
      tr("Enter a unique name or overwrite an existing search from the list:"));
...
void DatabaseWidget::onDatabaseModified() {
  ...
  int autosaveDelayMs = m_db->metadata()->autosaveDelayMin() * 60 *
                        1000; // min to msec for QTimer
  bool autosaveAfterEveryChangeConfig =
...
void DatabaseWidget::onAutosaveDelayTimeout() {
  ...
  const bool autosaveAfterEveryChangeConfig =
      config()->get(Config::AutoSaveAfterEveryChange).toBool();
...
QString DatabaseWidget::getCurrentSearch() { return m_lastSearchText; }
...
void DatabaseWidget::endSearch() {
  if (isSearchActive()) {
    ...
    m_entryView->setFirstEntryActive();
    // Enforce preview view update (prevents stale information if focus group is
    // empty)
...
bool DatabaseWidget::canCloneCurrentGroup() const {
  ...
  return currentGroup != m_db->rootGroup() &&
         currentGroup != m_db->metadata()->recycleBin();
}
...
bool DatabaseWidget::focusNextPrevChild(bool next) {
  // [parent] <-> GroupView <-> TagView <-> EntryView <-> EntryPreview <->
  // [parent]
  QList<QWidget *> sequence = {m_groupView, m_tagView, m_entryView,
                               m_previewView};
...
bool DatabaseWidget::lock() {
    ...
    if (!saved) {
      ...
      if (!m_db->metadata()->name().toHtmlEscaped().isEmpty()) {
        msg = tr("\"%1\" was modified.\nSave changes?")
                  .arg(m_db->metadata()->name().toHtmlEscaped());
      } else {
        msg = tr("Database was modified.\nSave changes?");
      }
      auto result = MessageBox::question(
          this, tr("Save changes?"), msg,
  ...
  } else if (m_db->hasNonDataChanges() &&
...
void DatabaseWidget::reloadDatabaseFile() {
  // Ignore reload if we are locked, saving, or currently editing an entry or
  // group
  ...
  if (db->open(database()->key(), &error)) {
    if (m_db->isModified() || db->hasNonDataChanges()) {
      ...
      auto result = MessageBox::question(
          ...
          tr("The database file has changed and you have unsaved changes.\nDo "
             "you want to merge your changes?"),
          MessageBox::Merge | MessageBox::Discard, MessageBox::Merge);
    ...
    showMessage(tr("Could not open the new database file while attempting to "
                   "autoreload.\nError: %1")
                    .arg(error),
                MessageWidget::Error);
    // Mark db as modified since existing data may differ from file or file was
    // deleted
    m_db->markAsModified();
  }
 ...
 */
void DatabaseWidget::restoreGroupEntryFocus(const QUuid &groupUuid,
...
bool DatabaseWidget::currentEntryHasUsername() {
  ...
  return !currentEntry->resolveMultiplePlaceholders(currentEntry->username())
              .isEmpty();
}
...
bool DatabaseWidget::currentEntryHasPassword() {
  ...
  return !currentEntry->resolveMultiplePlaceholders(currentEntry->password())
              .isEmpty();
}
...
bool DatabaseWidget::currentEntryHasUrl() {
  ...
  return !currentEntry->resolveMultiplePlaceholders(currentEntry->url())
              .isEmpty();
}
...
bool DatabaseWidget::currentEntryHasNotes() {
  ...
  return !currentEntry->resolveMultiplePlaceholders(currentEntry->notes())
              .isEmpty();
}
...
bool DatabaseWidget::currentEntryHasAutoTypeEnabled() {
  ...
  return currentEntry->autoTypeEnabled() &&
         currentEntry->groupAutoTypeEnabled();
}

GroupView *DatabaseWidget::groupView() { return m_groupView; }

EntryView *DatabaseWidget::entryView() { return m_entryView; }
...
bool DatabaseWidget::save() {
  ...
  if (performSave(errorMessage)) {
    ...
    m_blockAutoSave = false;
    m_autosaveTimer
        ->stop(); // stop autosave delay to avoid triggering another save
    return true;
  }
  ...
  if (m_saveAttempts > 2 && config()->get(Config::UseAtomicSaves).toBool()) {
    ...
    auto result = MessageBox::question(
        ...
        MessageBox::Disable | MessageBox::Cancel, MessageBox::Disable);
  ...
  showMessage(tr("Writing the database failed: %1").arg(errorMessage),
              MessageWidget::Error, true, MessageWidget::LongAutoHideTimeout);
...
bool DatabaseWidget::saveAs() {
  ...
  if (!QFileInfo::exists(oldFilePath)) {
    QString defaultFileName =
        config()->get(Config::DefaultDatabaseFileName).toString();
    oldFilePath = QDir::toNativeSeparators(
        FileDialog::getLastDir("db") + "/" +
        (defaultFileName.isEmpty() ? tr("Passwords").append(".kdbx")
                                   : defaultFileName));
  }
...
bool DatabaseWidget::performSave(QString &errorMessage,
  ...
  if (config()->get(Config::BackupBeforeSave).toBool()) {
    ...
    backupFilePath = Tools::substituteBackupFilePath(
        backupFilePath, dbFileInfo.canonicalFilePath());
    if (!backupFilePath.isNull()) {
      // Note that we cannot guarantee that backupFilePath is actually a valid
      // filename. QT currently provides no function for this. Moreover, we
      // don't check if backupFilePath is a file and not a directory. If this
      // isn't the case, just let the backup fail.
      if (QDir::isRelativePath(backupFilePath)) {
        backupFilePath = QDir::cleanPath(dbFileInfo.absolutePath() +
                                         QDir::separator() + backupFilePath);
      }
    }
  }
...
bool DatabaseWidget::saveBackup() {
  while (true) {
    ...
    if (!newFilePath.isEmpty()) {
      ...
      m_db->setFilePath(oldFilePath);
      if (modified) {
        // Source database is marked as clean when copy is saved, even if source
        // has unsaved changes
        m_db->markAsModified();
      }
...
void DatabaseWidget::emptyRecycleBin() {
  ...
  auto result = MessageBox::question(
      ...
      tr("Are you sure you want to permanently delete everything from your "
         "recycle bin?"),
      MessageBox::Empty | MessageBox::Cancel, MessageBox::Cancel);
...
void DatabaseWidget::openDatabaseFromEntry(const Entry *entry,
                                           bool inBackground) {
  ...
  // Request to open the database file in the background with a password and
  // keyfile
  emit requestOpenDatabase(dbFileInfo.canonicalFilePath(), inBackground,
                           password, keyFileInfo.canonicalFilePath());
}
