...
class SafariWebExtensionHandler: NSObject, NSExtensionRequestHandling {
    ...
    func connectSocket() -> Bool {
        ...
        guard setsockopt(Self.socketFD, SOL_SOCKET, SO_REUSEADDR, &optval, socklen_t(MemoryLayout<Int32>.size)) != -1 else {
            logger.error("setsockopt error: \(errno)")
            return false
        }

        guard setsockopt(Self.socketFD, SOL_SOCKET, SO_SNDBUF, &maxMessageLength, socklen_t(MemoryLayout<Int32>.size(ofValue: maxMessageLength))) != -1 else {
    ...
    func beginRequest(with context: NSExtensionContext) {
        ...
        if (!Self.socketConnected) {
            if (!connectSocket()) {
                ...
                context.cancelRequest(withError: SafariWebExtensionHandlerError.socketConnectionFailed)
                return
            }
